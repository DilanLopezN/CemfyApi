 ...
 * @author Dilan Lopez
 */
export async function rentValtCredit(
  ...
  try {
    ...
    const addressNameParts = assignee.address?.address_name?.match(/(\d+)/);
    const streetNumber = addressNameParts
      ? parseInt(addressNameParts[0], 10)
      : null;

    const billingAdress = {
      street: assignee.address?.address_name?.replace(/(\d+)/, '').trim(),
      number: streetNumber?.toString() || '',
      neighborhood: assignee.address?.neighborhood,
      zipcode: assignee.address?.zipcode,
